  ...
  <div class="main" id="index">
    ...
    <div class="basic-Info">
      <div class="top">
        ...
        <div class="sign">
          <div class="block" @click="!messageShow && checkIn()" v-if="!signInHide">
      ...
      <div class="content" id="content">
        <p @click="openBill(1)"><b>{{ contractNb || '-' }}</b><br/>賬單繳費</p>
        <p @click="openBill(2)"><b>{{ integralNum || '-' }}</b><br/>會員積分</p>
        <p @click="openBill(3)"><b>{{ giftNum || 0 }}</b><br/>我的禮品</p>
      </div>
    </div>
    <!-- 消息 -->
    <div class="message" v-if="messageShow">
      <i class="el-icon-error" @click="messageShow = false"></i>
      <p>註冊成為會員，隨時繳費及查詢最新資訊，每日領取會員積分，可獲得豐富禮品！</p>
    </div>
    <!-- 账单消息 -->
    <div class="billInfoBody" v-if="billInfoShow">
      <transition name="el-zoom-in-center" v-for="(item,index) in uRemindsList" :key="index">
        <div class="billInfo" v-if="item.billInfoShow">
          ...
          <h2><p>{{ item.aliasLb }}</p><span>● {{ item.billAmount <= 10 ? '已缴费' : '未繳費' }}</span></h2>
          <div class="number">
            <div class="bodyNb" v-if="item.viewFlag !== '102' || item.payable">合同號碼：<span>{{ item.warnBodyNb }}</span></div>
<!--            <router-link :to="'/bills/payment/' + item.warnBodyNb"><div class="fees no-touch">前往缴费<i class="el-icon-arrow-right"></i></div></router-link>-->
            <div class="fees no-touch" @click="$router.push('/bills/payment/' + item.warnBodyNb)">前往缴费<i class="el-icon-arrow-right"></i></div>
          </div>
          <p class="money">結欠金額：<span>MOP ${{ item.billAmount }}</span></p>
          <!-- 切换 -->
          <!-- <div class="switch">
      ...
      <div class="content">
        <p @click="routerHerf('/bills',true)">
          <el-badge is-dot class="item" :hidden="badgeShow">
            <svg-icon icon-class="wechat-bills"/><br/>微信賬單
          </el-badge>
        </p>
        <p @click="routerHerf('/bills')">
          <el-badge :value="uRemindsList.length" :hidden="uRemindsList.length==0?true:false" class="item">
            <svg-icon icon-class="management-bills"/><br/>管理賬單
          </el-badge>
        </p>
        <p @click="externalLink('/member/bill-management',true)"><svg-icon icon-class="sms"/><br/>短訊提示</p>
        <p @click="routerHerf('/other/quick-payment','','nologin')"><svg-icon icon-class="shortcut"/><br/>快捷繳費</p>
        <p @click="exLink('https://www.macaowater.com/about-macao-water/suspension-notice',false,'nologin')"><svg-icon icon-class="inform" /><br/>停水通知</p>
        <p @click="exLink('https://www.macaowater.com/about-macao-water/daily-water-quality-report',false,'nologin')"><svg-icon icon-class="water-quality"/><br/>每日水質</p>

        <p @click="exLink('https://www.macaowater.com/customer/tariffs-charge',false,'nologin')"><svg-icon icon-class="gas-prices"/><br/>水費價格</p>


        <p @click="routerHerf('/other/more?login='+userLoginAuto,'','nologin')"><svg-icon icon-class="more"/><br/>更多</p>
      </div>
    </div>
    <!-- 会员中心 -->
    <div class="member">
      <h2>會員中心</h2>
      <div class="content">
        <p @click="routerHerf('/member/gift')"><svg-icon icon-class="gift"/><br/>禮品兌換</p>
        <p @click="routerHerf('/member/integral')"><svg-icon icon-class="integral"/><br/>積分任務</p>
        <p @click="routerHerf('/member/membership')"><svg-icon icon-class="information"/><br/>會員資料</p>
      </div>
    </div>
    <!-- 今日水质 -->
    <div class="today">
    ...
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        <el-button round @click="helpShow = false">關閉</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button round type="primary"><router-link to="/bills">去绑定</router-link></el-button>
      </span>
    </el-dialog>
