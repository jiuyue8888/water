<template>
  <div class="main">
    <!-- 页头 -->
    <Header :metaTitle='$t("billsDetails.font1")' :bodyNb="bodyNb" />
    <!-- 主体内容 -->
    <div class="body">
      <!-- 账单AMR -->
      <div class="amr" v-if="billDetails.registerState && amrShow">
        <div class="title">{{$t("billsDetails.font2")}}</div>
            ...
            <el-col :span="16">
              <ul>
                <el-row :gutter="20">
				  ...
				  {{ formatNumberRgx(this.chartDataText.lastWeek) }}L<p>{{$t("billsDetails.font4")}}</p></li></el-col>
                  <el-col :span="12"><li>{{ formatNumberRgx(this.chartDataText.monthWeek) }}L<p>{{$t("billsDetails.font5")}}</p></li></el-col>
                </el-row>
              </ul>
            </el-col>
            <el-col :span="8">
              <div class="right"><ve-pie :legend-visible="false" :data="chartData" :settings="chartSettings" :colors="colors" height="130px">
			  </ve-pie></div>
            </el-col>
          </el-row>
        </div>
        <div class="title" style="padding-top: 10px;">{{$t("billsDetails.font6")}}</div>
          ...
          <el-row>
            <el-col :span="8">
              <div class="table-state">
                <div style="color: #EA3323;" v-if="amrNumber<20">{{$t("billsDetails.font7")}}</div>
                <div style="color: #EA983E;" v-else-if="amrNumber<40">{{$t("billsDetails.font8")}}</div>
                <div style="color: #FDF151;" v-else-if="amrNumber<60">{{$t("billsDetails.font9")}}</div>
                <div style="color: #99C355;" v-else-if="amrNumber<80">{{$t("billsDetails.font10")}}</div>
                <div style="color: #4AA35A;" v-else>{{$t("billsDetails.font11")}}</div>
              </div>
              <div class="calc-div">
            ...
            <el-col :span="16">
				  ...
				  <div :class="'table-box '+$t('image.img1')">
        ...
        <p class="more">{{ this.chartDataText.updateDate }}<span @click="warningShow = !warningShow">{{$t("billsDetails.font12")}} 
		<i :class="warningShow == true ? 'el-icon-arrow-up' : 'el-icon-arrow-down'"></i></span></p>
      </div>
      <!-- 警告推送 -->
      <el-collapse-transition v-if="billDetails.registerState && warningShow">
        <div class="warning">
          ...
          <div class="switch">
            {{$t("billsDetails.font16")}}
            <div class="right">
              <i class="el-icon-warning-outline"></i>
              <span class="text">{{$t("billsDetails.font17")}}</span>
              <el-switch
                      :value="warningSwitch"
          ...
          <div class="tab">
            ...
            <el-tabs v-model="tabName">
              <el-tab-pane :label='$t("billsDetails.font18")' name="first">
                <el-timeline>
                  <el-timeline-item
      ...
      <div class="info">
        <div class="center">
          <div class="title"><div class="over-hidden"><p>{{ billDetails.bodyName || billDetails.bodyNb }}</p>
		  <span class="yellow" v-if="!billDetails.registerState">{{$t("bills.font3")}}</span></div><span :class="billDetails.amount <= 10?'green':'red'">● 
		  {{ billDetails.amount <= 10 ? $t("billsDetails.font22") : $t("billsDetails.font23") }}</span></div>
          <div style="border-bottom: 1px solid rgba(92,141,230,0.4);">{{$t("billsDetails.font19")}}：<span>{{ billDetails.deadline }}</span></div>
          <div style="border-bottom: 1px solid rgba(92,141,230,0.4);">{{$t("billsDetails.font20")}}：<span>{{ billDetails.avoidDeadline }}</span></div>
          <div>{{$t("billsDetails.font21")}}：<span>MOP <span class="red">${{ billDetails.amount }}</span></span></div>
        </div>
      </div>
      <!-- 账单服务 -->
      <div class="serve">
        <el-collapse v-model="activeNames" @change="collapseChange" ref="collapse">
          <el-collapse-item :title='$t("billsDetails.font24")' name="1">
            <ul>
              <li v-if="billDetails.registerState">
                <div><svg-icon icon-class="wechat"></svg-icon>{{$t("billsDetails.font25")}}<div class="brandNew">{{$t("billsDetails.font26")}}</div></div>
                <el-switch
              ...
              <li @click="editShow = true">
                <div><svg-icon icon-class="edit"></svg-icon>{{$t("billsDetails.font27")}}</div>
                <i class="el-icon-arrow-right"></i>
              </li>
              <li v-if="billDetails.registerState" @click="externalLink('/member/bill-management',true)">
                <div><svg-icon icon-class="note"></svg-icon>{{$t("billsDetails.font28")}}</div>
                <i class="el-icon-arrow-right"></i>
              </li>
              <li v-if="billDetails.registerState && amrShow">
                <div><svg-icon icon-class="warning"></svg-icon>{{$t("billsDetails.font29")}}</div>
                <el-switch
                        :value="warningSwitch"
                        @change="switchChange($event,2)"
                        active-color="#9DCB44"
          ...
          <el-collapse-item :title='$t("billsDetails.font30")' name="2">
            <ul>
              <li @click="openPayment">
                <div><svg-icon icon-class="money"></svg-icon>{{$t("billsDetails.font31")}}</div>
                <div class="payment">{{$t("billsDetails.font32")}}<i class="el-icon-arrow-right"></i></div>
              </li>
              <!-- <li @click="openQrCode">
          ...
          <el-collapse-item :title='$t("billsDetails.font33")' name="3">
            ...
            <ul v-else>
              ...
              <li class="no-touch" @click="$router.push({path: '/bills/pdf', query: { bodyNb: bodyNb, transcNb: billDetails.transcNb, registerState: billDetails.registerState }})">
                <div>{{ billDetails.porderMonth }}
                  <div class="brandNew" style="color: #9DCB44;border:1px solid #9DCB44;">{{$t("billsDetails.font34")}}</div>
                </div>
    ...
    <el-dialog
      ...
      <el-form  class="contractForm" ref="form" :rules="rules" :model="form" label-width="95px">
        ...
        <el-form-item :label='$t("billsDetails.font37")' class="onUnderline">
          <el-input readonly class="underline" v-model="billDetails.bodyName"></el-input>
        </el-form-item>
        <!--        prop="bodyName"-->
        <el-form-item :label='$t("billsDetails.font38")' class="new-alias">
          <el-input class="underline" maxlength="64" v-model="form.bodyName" :placeholder='$t("billsDetails.font39")'></el-input>
        </el-form-item>
      </el-form>
      <!-- 底部 -->
      <span slot="footer" class="dialog-footer">
        <el-button round @click="editShow = false">{{$t("bills.font15")}}</el-button>
        <el-button round type="primary" @click="editSubmit">{{$t("billsDetails.font40")}}</el-button>
      </span>
    </el-dialog>
  ...
  export default {
    ...
    methods:{
      ...
      async getBillDetails(){
        let billDetails = Object.keys(this.$route.params).length ? this.$route.params :
		 JSON.parse(sessionStorage.billDetails);
      ...
      async getWater(){
        ...
        if(res.code == 200){
          ...
          if(res.result !== null){
            ...
            let arr = [
              {text : this.$t('billsDetails.font3'), value : res.result.thisWeekConsumption},
              {text : this.$t('billsDetails.font4'), value : res.result.lastWeekConsumption},
              {text : '', value : numTwo},
            ];
            ...
            this.chartDataText.updateDate = parseTime(res.result.lastReadingDt,this.$t('billsMessage.font1')+'{m}-{d} {h}:{i}');
            if(num > 0){
              this.colors[2] = '#FD5252';
              arr[2].text = this.$t('billsDetails.font44');
              this.chartDataText.calcText = this.$t('billsDetails.font44');
            }else{
              this.colors[2] = '#fff';
              arr[2].text = this.$t('billsDetails.font45');
              this.chartDataText.calcText = this.$t('billsDetails.font45');
            }
      ...
      async switchChange(state,type){
        ...
        let content =this.$t('billsDetails.font48');
		if(type == 1){
          this.dialogContent.title = this.$t('billsDetails.font46');
          this.dialogContent.content = content;
        }else if(type == 2){
          if (this.cdType === '1' && this.cdStatus === '1') {
            this.dialogContent.title = this.$t('billsDetails.font47');
            this.dialogContent.cdType = '1';
            this.dialogContent.cdStatus = '1';
          } else {
            this.dialogContent.title = this.$t('billsDetails.font52');
          }
		  this.dialogContent.content = this.$t('billsDetails.font49')
...
<style lang="scss" scoped>
  ...
  .rank-div{
	...
	.img2{
		background: url("../../assets/images/table-en.png") no-repeat;
		background-size: 100%;
	}
  }
