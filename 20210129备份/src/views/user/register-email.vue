<template>
	<div class="main">
		...
		<div class="body">
			...
			<div class="next-step">
				<div class="statement">
					<el-checkbox v-model="statementCbox">{{ $t('registerEmail.font6') }}</el-checkbox>
					<span class="text" @click="lockStatement">
						{{ $i18n.locale == 'ZH' ? '《' : '《' }}{{ $t('registerPhone.font17') }}{{ $i18n.locale == 'ZH' ? '》' : '》' }}
					</span>
					<br />
					<span class="text" @click="electronicShow = true">
						{{ $i18n.locale == 'ZH' ? '《' : '《' }}{{ $t('registerPhone.font19') }}{{ $i18n.locale == 'ZH' ? '》' : '》' }}
					</span>
				</div>
			...
			<el-dialog
				...
				center
			>
				<el-card class="box-card" shadow="never" :body-style="{ padding: '10px' }"><div class="text item" v-html="$t('registerPhone.font18')"></div></el-card>
			...
			<el-dialog
				...
				center
			>
				<el-card class="box-card" shadow="never" :body-style="{ padding: '10px' }"><div class="text item" v-html="$t('billsPayment.font19')"></div></el-card>
				<span slot="footer" class="dialog-footer">
	...
	methods: {
		...
		async onRegister(obj) {
			let phone = window.localStorage.getItem('zcphone'),
				...
				phoneRegionId = '',
				areaCode = window.localStorage.getItem('zareaCode');
			let data = {
				openId: getToken(),
				userPhone: phone,
