...
if(process.env.NODE_ENV === 'production'){  // 生产环境
    target = envConfig.stageEnv;
}else if(process.env.NODE_ENV === 'staging'){ // 测试环境
    // target = envConfig.stageEnv;
    target = location.protocol + '//' + location.hostname;
}else{  // 本地环境
    // target = envConfig.stageEnv;
    target = envConfig.prodEnv;
    // target = envConfig.lanEnv;
    // target = envConfig.lanEnv2;
...
module.exports = {
  ...
  chainWebpack(config) {
    const cdn = {
      ...
      // 为什么使用这个cdn，详情见 https://github.com/PanJiaChen/tinymce-all-in-one
